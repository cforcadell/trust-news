...
Genera un plan con un máximo solicitado de 0,05 USD y un margen del 5 %:
    ...
    python3 scripts/llm-benchmark.py generate-profiles \
      ...
      --output-root artifacts/llm-benchmark/generated

El máximo efectivo será 0,0475 USD. El comando imprime una ruta como:

    LLM_PROFILE_PLAN artifacts/llm-benchmark/generated/openrouter-plan-.../plan.json

Guarda esa ruta en una variable:

    export PLAN_PATH='artifacts/llm-benchmark/generated/openrouter-plan-.../plan.json'
...
La validación no modifica configuración ni crea órdenes:
    ...
    for profile in "$PLAN_DIR"/profiles/*.json; do
      python3 scripts/llm-benchmark.py validate-profiles \
        --case benchmarks/llm/cases/eu-news-2025-v1.json \
        --profile "$profile"
    done
...
primera ejecución:
    ...
    python3 scripts/llm-benchmark.py run \
      ...
      --repetitions 5 \
      --require-costs \
      --artifacts-root artifacts/llm-benchmark \
      --database artifacts/llm-benchmark/history.sqlite
...
Lista las ejecuciones guardadas:
    ...
    python3 scripts/llm-benchmark.py list-runs \
      --database artifacts/llm-benchmark/history.sqlite \
      --limit 30

Los informes se guardan en:

    artifacts/llm-benchmark/<batch-id>/report.md
    artifacts/llm-benchmark/<batch-id>/summary.json
