...
def test_publish_new_basic():
    """
    Prueba de integración básica del endpoint /publishNew.
    ...
    if response.status_code == 200:
        ...
        print(f"✅ publishNew OK: postId={data['postId']}, tx={data['tx_hash']}")
    else:
        print(f"⚠️ publishNew devolvió 500 (probablemente blockchain no accesible): {response.text}")
...
def test_get_new_by_hash_and_cid():
    ...
    print(f"📘 getNewByHash status={res_hash.status_code}, getNewByCid status={res_cid.status_code}")


@pytest.mark.integration
def test_get_asertions_with_validations():
    """
    Prueba de integración del endpoint /getAsertionsWithValidations/{PostId}
    """
    postId = 1
    ...
    if response.status_code == 200:
        ...
        print(f"✅ getAsertionsWithValidations OK para PostId={postId}")
    else:
        print(f"⚠️ No se pudo acceder al contrato (500): {response.text}")
